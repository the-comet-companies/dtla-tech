import { heroBentoCards } from "@/lib/constants";
...
const featureIcons: React.ReactNode[] = [
  // Production Queue
  <svg key="pq" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z" />
  </svg>,
  // Quick Quote
  <svg key="qq" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
  </svg>,
  // Order Management
  <svg key="om" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15a2.25 2.25 0 012.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z" />
  </svg>,
  // AI Agents
  <svg key="ai" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
type Card = FeatureCardData | LogoCardData;

// Merge constants data with local icons
type FeatureData = (typeof heroBentoCards.features)[number];
type LogoData = (typeof heroBentoCards.logos)[number];

const cards: Card[] = [
  // Row 1 — feature cards
  ...heroBentoCards.features.slice(0, 2).map((f: FeatureData, i: number) => ({
    type: "feature" as const,
    ...f,
    icon: featureIcons[i],
  })),
  // Row 2 — logo cards
  ...heroBentoCards.logos.slice(0, 2).map((l: LogoData) => ({
    type: "logos" as const,
    ...l,
  })),
  // Row 3 — feature cards
  ...heroBentoCards.features.slice(2, 4).map((f: FeatureData, i: number) => ({
    type: "feature" as const,
    ...f,
    icon: featureIcons[i + 2],
  })),
  // Row 4 — logo cards
  ...heroBentoCards.logos.slice(2, 4).map((l: LogoData) => ({
    type: "logos" as const,
    ...l,
  })),
];
